...
This is a specific implementation of [Offline Validation]({{< ref "/content/docs/license-lifecycle" >}}) that uses the USB drive as a hardware anchor.
...
### 3. State Enforcement
Even without a connection, the [License State Machine]({{< ref "/content/docs/license-lifecycle" >}}) is enforced. The embedded `expires_at` and `status` fields determine whether the software continues to run.
...
### 1. Create a Product
First, ensure your [Product]({{< ref "/content/docs/admin-portal/products" >}}) is configured in the Admin Portal with "Offline Support" enabled.

### 2. Generate the Offline License
Use the [Offline License Generator]({{< ref "/content/docs/admin-portal/generators" >}}) or the API to create a signed license file. You will need to provide the target Hardware ID (HWID) or USB identifier if binding is required.
...
*   [License Lifecycle]({{< ref "/content/docs/license-lifecycle" >}}) — Understand the different states of a license.
*   [Generators]({{< ref "/content/docs/admin-portal/generators" >}}) — How to create offline licenses at scale.
*   [Products]({{< ref "/content/docs/admin-portal/products" >}}) — Configuring your products for offline use.
*   [Activations]({{< ref "/content/docs/admin-portal/activations" >}}) — Managing existing license activations.
